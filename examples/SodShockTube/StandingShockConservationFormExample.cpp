...
//  double E_b   = (u_b * u_b) / 2 + p_b / ( (gamma-1.) * rho_b);
  ...
  if (interpolatedStart)
  {
    ...
    initialGuess[form->E()->ID()]     = rho * (Cv * T + 0.5 * u * u);
    initialGuess[form->m(1)->ID()]    = u * rho;
  ...
  {
    // cheating!  Instead of interpolating, let's start at what we know to be close to the right answer, at least for rho, T, u
  ...
  HDF5Exporter solutionExporter(form->solutionIncrement()->mesh(), "standingShockSolution", ".");
  HDF5Exporter solutionIncrementExporter(form->solutionIncrement()->mesh(), "standingShockSolutionIncrement", ".");
