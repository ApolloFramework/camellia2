...
//      print("allCells", cellsToReportOn);
  ...
  void testSaveAndLoad2D(BFPtr bf, Teuchos::FancyOStream &out, bool &success)
  {
    ...
    //TMP soln->save(filePrefix);
    
    //TMP soln->load(bf, filePrefix);
    //TMP MeshPtr loadedMesh = soln->mesh();
    //TMP TEST_EQUALITY(loadedMesh->globalDofCount(), mesh->globalDofCount());
    
    // delete the files we created
    //TMP remove((filePrefix+".soln").c_str());
    //TMP remove((filePrefix+".mesh").c_str());
    
    // just to confirm that we can manipulate the loaded mesh:
    //TMP set<GlobalIndexType> cellsToRefine;
    //TMP cellsToRefine.insert(0);
    //TMP loadedMesh->pRefine(cellsToRefine);
  }
