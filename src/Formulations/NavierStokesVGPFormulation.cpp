...
//  LinearTermPtr residual = _rhsForResidual->linearTerm(); // post-accumulation, the RHS is the whole residual
//
//  _refinementStrategy = Teuchos::rcp( new RefinementStrategy(mesh, residual, _solnIncrement->ip(), energyThreshold) );
//  _refinementStrategy->setRieszRepCubatureEnrichmentDegree(spatialPolyOrder);
//
//  double maxDouble = std::numeric_limits<double>::max();
//  double maxP = 20;
//  _hRefinementStrategy = Teuchos::rcp( new RefinementStrategy( mesh, residual, _solnIncrement->ip(), energyThreshold, 0, 0, false ) );
//  _pRefinementStrategy = Teuchos::rcp( new RefinementStrategy( mesh, residual, _solnIncrement->ip(), energyThreshold, maxDouble, maxP, true ) );
//  _pRefinementStrategy->setRieszRepCubatureEnrichmentDegree(spatialPolyOrder); // maxP might be better, but this might also be overkill...
//  _hRefinementStrategy->setRieszRepCubatureEnrichmentDegree(spatialPolyOrder);
...
void NavierStokesVGPFormulation::refineUniformly()
{
  _nonlinearIterationCount = 0;
  this->clearSolutionIncrement();
  _solnIncrement->setRHS(_rhsForResidual);
